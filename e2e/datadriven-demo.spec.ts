      ...
      try {
        ...
        if (testCase.DOB) {
          ...
          if (/^\d{4}-\d{2}-\d{2}$/.test(iso)) {
            ...
            console.log(
              `Test ${testCase.TestCaseID}: Skipping invalid DOB value: ${testCase.DOB}`
            );
          }
        }
        if (testCase.Street)
          await page
            .locator('input[data-test="address"]')
            .fill(testCase.Street);
        if (testCase.PostalCode)
          await page
            .locator('input[data-test="postcode"]')
            .fill(testCase.PostalCode);
        if (testCase.City)
          await page.locator('input[data-test="city"]').fill(testCase.City);
        if (testCase.State)
          await page.locator('input[data-test="state"]').fill(testCase.State);
        ...
        if (testCase.Country) {
          ...
          const options = await countrySelect.evaluate((select) =>
          ...
          if (options.includes(code)) {
            ...
            console.log(
              `Test ${testCase.TestCaseID}: Skipping invalid country code: ${code}`
            );
          }
        }
        if (testCase.Phone)
          await page.locator('input[data-test="phone"]').fill(testCase.Phone);
        if (testCase.Email)
          await page.locator('input[data-test="email"]').fill(testCase.Email);
        if (testCase.Password)
          await page.locator('input[type="password"]').fill(testCase.Password);
        ...
        const successVisible = await successLocator
          .isVisible()
          .catch(() => false);
        if (errorVisible) {
          const errorText = await errorLocator.innerText();
          console.log(
            `Test ${testCase.TestCaseID} failed as expected: ${errorText}`
          );
          ...
          console.log(
            `Test ${testCase.TestCaseID}: No visible error or success message after submit.`
          );
        }
      } catch (e) {
